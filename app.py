...
# ============================================
# FUNCAO DE PARSE DO TEXTO BRUTO - V2 ROBUSTA
# ============================================
def parse_texto_bruto(texto):
    """Extrai dados do texto da rota usando regex e heuristicas."""
    dados = {}
    ...
    # --- 3. Identificar linhas ja "usadas" ---
    linhas_usadas = set()

    for i, linha in enumerate(linhas):
        # Data
        if dados['data'] and dados['data'] in linha:
            linhas_usadas.add(i)
            continue
        # Parada
        if dados['parada'] and f"Parada {dados['parada']}" in linha:
            linhas_usadas.add(i)
            continue
        # PC/NF
        if dados['pc'] and dados['pc'] in linha and ('NF' in linha or 'ITENS' in linha or 'Download' in linha):
            linhas_usadas.add(i)
            continue
        # Modelo
        if dados['modelo'] and f"Modelo: {dados['modelo']}" in linha:
            linhas_usadas.add(i)
            continue
        # Chamado
        if dados['chamado'] and f"Chamado: {dados['chamado']}" in linha:
            linhas_usadas.add(i)
            continue
        # SAP
        if dados['sap'] and f"SAP: {dados['sap']}" in linha:
            linhas_usadas.add(i)
            continue
        # Territorio
        if dados['territorio'] and f"Territorio: {dados['territorio']}" in linha:
            linhas_usadas.add(i)
            continue
        # Telemetria
        if dados['telemetria'] and f"Possui Telemetria: {dados['telemetria']}" in linha:
            linhas_usadas.add(i)
            continue
        # Endereco
        if dados['endereco'] and dados['endereco'] in linha:
            linhas_usadas.add(i)
            continue
        # Tipo de atividade
        if i == tipo_idx:
            linhas_usadas.add(i)
            continue
        # Palavras-chave de exclusao
        excluir = ['Esconder', 'Download', 'Mapa', 'Comentario', 'COMENTARIO', 'AGENDAMENTO', 'LEVAR', 'ARMARIO']
        if any(exc.lower() in linha.lower() for exc in excluir):
            linhas_usadas.add(i)
            continue
        # Muito curta ou so numeros
        if len(linha) <= 2 or linha.isdigit():
            linhas_usadas.add(i)
            continue
        # Endereco (comeca com prefixos)
        if linha.startswith('R ') or linha.startswith('Av') or linha.startswith('Rua') or linha.startswith('Avenida'):
            linhas_usadas.add(i)
            continue

    # --- 4. Candidatos a cliente e cidade ---
    candidatos = []
    for i, linha in enumerate(linhas):
    ...
    # --- 5. Heuristica: cliente = mais longo, cidade = mais curto ---
    dados['cliente'] = ''
    dados['cidade'] = ''

    if len(candidatos) >= 2:
        # Ordenar por tamanho decrescente
        candidatos_ordenados = sorted(candidatos, key=lambda x: len(x[1]), reverse=True)
        dados['cliente'] = candidatos_ordenados[0][1]
        # Cidade = proximo candidato mais curto que nao seja igual ao cliente
        for idx, linha in candidatos_ordenados[1:]:
            if linha != dados['cliente'] and len(linha) < 50:
                dados['cidade'] = linha
                break
    elif len(candidatos) == 1:
        dados['cliente'] = candidatos[0][1]
